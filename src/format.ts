import type { AgentResource, LintIssue, OutputFormat, ReportData, SearchHit } from "./types.js";
...
function resourceCard(resource: AgentResource, score?: number, matchedTerms: readonly string[] = []): string {
  const searchable = [resource.name, resource.kind, resource.description, resource.tags.join(" "), resource.path].join(" ").toLocaleLowerCase("en-US");
...
function htmlReport(report: ReportData): string {
  const cards = report.hits?.map((hit) => resourceCard(hit.resource, hit.score, hit.matchedTerms)).join("")
